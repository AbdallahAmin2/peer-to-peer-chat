...
class PeerClient(threading.Thread):
    ...
    def run(self):
        ...
        print(f'{Fore.GREEN}Chatroom joined Successfully. \nType to start chatting'
...
class peerMain:
    ...
    def __init__(self, username=None):
        ...
        self.registryName = input(f"{Fore.YELLOW}Enter IP address of registry: {Fore.LIGHTRED_EX}")
        ...
        while choice != "3":
                ...
                if choice == "1" and not self.isOnline:
                    username = input(f"username: {Fore.LIGHTMAGENTA_EX}")
                    password = input(f"{Fore.RESET}password: {Fore.LIGHTMAGENTA_EX}")
                    self.createAccount(username, password)
                # if choice is 2 and user is not logged in, asks for the username
                # and the password to login
                elif choice == "2" and not self.isOnline:
                    username = input(f"{Fore.RESET}username: {Fore.LIGHTMAGENTA_EX}")
                    password = input(f"{Fore.RESET}password: {Fore.LIGHTMAGENTA_EX}")
                ...
                # for a username wanted to be searched
                    ...
                    username = input("Username you want to find: ")
                ...
                # to enter the username of the user that is wanted to be chatted
                    ...
                    username = input("Enter username you want to chat: ")
                    searchStatus = self.searchUser(username)
                ...
                # Creating chat room
                    ...
                    chat_name = input(f"Enter the Chat room name: {Fore.LIGHTYELLOW_EX}")
                    self.create_chatroom(chat_name)
                # Joining Chat room
                elif choice == "7":
                    chat_name = input(f"Enter the Chat room name to Join: {Fore.LIGHTYELLOW_EX}")
                    self.join_chatroom(chat_name)
    ...
    def create_chatroom(self, chat_room_name):
        message = "CREATECHATROOM " + chat_room_name
        self.tcpClientSocket.send(message.encode())
        response = self.tcpClientSocket.recv(1024).decode()
        if response.startswith("createdSucc"):
            creator = response[len("createdSucc "):]
            print(f"{Fore.LIGHTCYAN_EX}Chat room {chat_room_name} was created successfully by {creator}{Fore.RESET}")
            self.join_chatroom(chat_room_name)
        else:
