...
class PeerServer(threading.Thread):
    ...
    def run(self):
        ...
        print(f"{Fore.RED}{Style.BRIGHT}Peer server started...{Fore.RESET}")    
                    ...
                    # is used to communicate with a connected peer, then enters here
                        ...
                        if len(messageReceived) > 11 and messageReceived[:12] == "CHAT-REQUEST":
                            ...
                            if s is self.connectedPeerSocket:
                                ...
                                print(f"{Fore.LIGHTCYAN_EX}Incoming chat request from{Fore.RESET}" + self.chattingClientName + f"{Fore.LIGHTCYAN_EX} >> {Fore.RESET}")
                                print(f"{Fore.LIGHTCYAN_EX}Enter OK to accept or REJECT to reject:  {Fore.RESET}")
                        ...
                        # removes the socket of the connected peer from the inputs list
                            ...
                            if len(messageReceived) == 2:
                                print(f"{Fore.LIGHTMAGENTA_EX}User you're chatting with ended the chat{Fore.RESET}")
                                print(f"{Fore.LIGHTMAGENTA_EX}Press enter to quit the chat:{Fore.RESET} ")
                        ...
                        # connected user suddenly ended the chat(an error occurred)
                            ...
                            inputs.clear()
                            inputs.append(self.tcpServerSocket)
                            print(f"{Fore.LIGHTRED_EX}User you're chatting with suddenly ended the chat{Fore.RESET}")
                            print(f"{Fore.LIGHTYELLOW_EX}Press enter to quit the chat: {Fore.RESET}")
...
class PeerClient(threading.Thread):
    ...
    def run(self):
        print(f"{Fore.LIGHTRED_EX}Peer client started...{Fore.RESET}")
        ...
        if self.peerServer.isChatRequested == 0 and self.responseReceived is None:
            ...
            requestMessage = "CHAT-REQUEST " + str(self.peerServer.peerServerPort)+ " " + self.username + f"{Fore.LIGHTGREEN_EX} \n [In Case you want to end chat and return to menu type :q hope you enjoy it ]{Fore.RESET}"
            ...
            print(f"{Fore.LIGHTMAGENTA_EX}Request message{Fore.RESET} " + requestMessage + f"{Fore.LIGHTMAGENTA_EX} is sent...{Fore.RESET}")
            ...
            print(f"{Fore.LIGHTMAGENTA_EX}Response is {Fore.RESET}" + self.responseReceived)
            ...
            # logs the message and then the socket is closed       
                ...
                self.peerServer.isChatRequested = 0
                print(f"{Fore.LIGHTRED_EX}Client requested is closing...{Fore.RESET}")
            ...
            # if a busy response is received, closes the socket
                ...
                print(f"{Fore.LIGHTBLUE_EX}Receiver peer is busy{Fore.RESET}")
                self.tcpClientSocket.close()
        ...
        # so it sends an OK message to the requesting side peer server that it connects and then waits for the user inputs.
            ...
            print(f"{Fore.LIGHTYELLOW_EX}Client with OK message is created... and sending messages{Fore.RESET}")
...
class peerMain:
    ...
    def __init__(self):
        ...
        self.registryName = input(f"{Fore.LIGHTYELLOW_EX}Enter IP address of registry: {Fore.RESET}")
        ...
        while 1:
            ...
            choice = input(f"{Fore.LIGHTMAGENTA_EX}Choose: \nCreate account: 1\nLogin: 2\nQUIT : Q\n{Fore.RESET}")
            ...
            if not self.isOnline:
                ...
                # and the password to login
                    ...
                    username = input(f"{Fore.LIGHTCYAN_EX}username: {Fore.RESET} ")
                    password = input(f"{Fore.LIGHTCYAN_EX}password: {Fore.RESET} ")
                    self.username=username
                    # asks for the port number for server's tcp socket
                    peerServerPort = int(input(f"{Fore.LIGHTCYAN_EX}Enter a port number for TCP peer server:{Fore.RESET} "))
                    UDP_Port = int(input(f"{Fore.LIGHTCYAN_EX}Enter a port number for UDP peer server: {Fore.RESET}"))
                    ...
                    print(f"{Fore.LIGHTBLUE_EX}You Exited the program...See You Soon{Fore.RESET}" + f"{Fore.LIGHTYELLOW_EX} :){Fore.RESET}")
                    break

                else:
                    print(f"{Fore.LIGHTBLUE_EX}\nInvalid Choice! Please try again.{Fore.RESET}")
            ...
            if(self.isOnline):
                ...
                while choice != "3":
                    ...
                    if choice == "3" and self.isOnline:
                        ...
                        print(f"{Fore.LIGHTCYAN_EX}Logged out successfully{Fore.RESET}")
                        break   #To return to the menu to login again if he changed his mind
                    ...
                    # for a username that is wanted to be searched
                        ...
                        username = input(f"{Fore.LIGHTMAGENTA_EX}Username to be searched: {Fore.RESET} ")
                    ...
                    # to enter the username of the user that is wanted to be chatted
                        ...
                        username = input(f"{Fore.LIGHTGREEN_EX}Enter the username of user to start chat: {Fore.RESET}")
                        searchStatus = self.searchUser(username)
                    ...
                    # if choice is cancel timer for hello message is cancelled
                        ...
                        roomname = input(f"{Fore.LIGHTCYAN_EX}enter room name: {Fore.RESET} ")
                        ...
                        self.show_rooms()
                         ...
                         print(f"{Fore.LIGHTCYAN_EX}the available rooms are :  {Fore.RESET}")
                         self.show_rooms()
                         roomid = input("enter room id: ")
                         exist = self.check_room_id(roomid)
                         while(exist == 0):
                            print(f"{Fore.LIGHTRED_EX}The room doesn't existf{Fore.RESET}")
                            roomid = input(f"{Fore.LIGHTRED_EX}enter correct room id: {Fore.RESET}")
                            exist = self.check_room_id(roomid)
                         usname = self.username
                         if self.is_userIn_room(roomid,self.username) == "True" :
                             print(f"{Fore.LIGHTCYAN_EX}You are already in This room...{Fore.RESET}")
                         else :
                            self.join_chatroom(roomid, usname)
                    # asks for the port number for server's tcp socket
                    elif choice == "9":
                        print(f"{Fore.LIGHTCYAN_EX}you are a member of this rooms :\n{Fore.RESET}"+self.show_rooms_for_user(self.username) )
                        roomid = input(f"{Fore.LIGHTCYAN_EX}enter room id: {Fore.RESET}")
                        print(f"{Fore.LIGHTGREEN_EX}you are now in room: {Fore.RESET}" + roomid + f"{Fore.LIGHTRED_EX} [to exit the chat room at any time, press: q]{Fore.RESET}")
                    ...
                    #leave room
                        ...
                        exist = self.check_room_id(roomid)
                        while(exist == 0):
                            print(f"{Fore.LIGHTRED_EX}The room doesn't exist{Fore.RESET}")
                            roomid = input(f"{Fore.LIGHTCYAN_EX}enter correct room id: {Fore.RESET}")
                            exist = self.check_room_id(roomid)
                        if(self.is_userIn_room(roomid,username) == "True"):
                            self.leaveRoom(roomid, username)
                        else:
                            print(f"{Fore.LIGHTMAGENTA_EX}You are not a member of the room.{Fore.RESET}")

                    else :
                        print(f"{Fore.LIGHTRED_EX}\nInvalid Option.\nPlease Enter Again.{Fore.RESET}")
